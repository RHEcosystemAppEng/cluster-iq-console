  ...
  const handleConfirm = () => {
    if (modalAction) {
      ...
      if (modalAction === PowerAction.POWER_ON) {
        startCluster(clusterID, userEmail, showReasonField ? reason : undefined);
        console.log('Powering on the cluster');
      } else if (modalAction === PowerAction.POWER_OFF) {
        stopCluster(clusterID, userEmail, showReasonField ? reason : undefined);
          ...
          <Stack hasGutter>
            ...
            <StackItem>
              <Checkbox
                ...
                isChecked={showReasonField}
                onChange={(_event, checked) => setShowReasonField(checked)}
                id="specify-reason"
              />
            </StackItem>
